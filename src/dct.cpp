// src/dct.cpp
// Tổng quát hoá bản 8x8 cũ (commit a591d9d) cho N = 4..32.
// Tham khảo HM: TComTrQuant.cpp (xTrMxN, xITrMxN), TComRom.cpp (g_aiT32).
#include "dct.h"
...
int log2Size(int N) {
    ...
    assert(false && "DCT chi ho tro N = 4, 8, 16, 32");
    return 0;
}

// Cả ma trận N x N, phần tử [k * N + n]. Tính một lần cho mỗi block thay vì gọi dctCoef trong vòng lặp.
std::vector<int32_t> dctMatrix(int N) {
    std::vector<int32_t> T(N * N);
    for (int k = 0; k < N; ++k) {
        for (int n = 0; n < N; ++n) {
...
int64_t roundShift(int64_t value, int shift) {
    return (value + (1LL << (shift - 1))) >> shift;   // dịch phải có làm tròn
}
...
std::vector<int32_t> forwardDct(const std::vector<int32_t>& residual, int N) {
    const int log2N  = log2Size(N);
    const int shift1 = log2N + kBitDepth - 9;   // N=16: 3
    const int shift2 = log2N + 6;               // N=16: 10
    const std::vector<int32_t> T = dctMatrix(N);

    std::vector<int32_t> A(N * N);
    for (int r = 0; r < N; ++r) {
        for (int c = 0; c < N; ++c) {
            int64_t sum = 0;
            for (int i = 0; i < N; ++i) sum += static_cast<int64_t>(residual[r * N + i]) * T[c * N + i];
            A[r * N + c] = static_cast<int32_t>(roundShift(sum, shift1));
        }
    }

    std::vector<int32_t> Y(N * N);
    for (int r = 0; r < N; ++r) {
        for (int c = 0; c < N; ++c) {
            int64_t sum = 0;
            for (int i = 0; i < N; ++i) sum += static_cast<int64_t>(T[r * N + i]) * A[i * N + c];
            Y[r * N + c] = static_cast<int32_t>(roundShift(sum, shift2));
        }
    }
    return Y;
}
...
std::vector<int32_t> inverseDct(const std::vector<int32_t>& coeff, int N) {
    log2Size(N);
    const int shift1 = 7;
    ...
    for (int r = 0; r < N; ++r) {
        for (int c = 0; c < N; ++c) {
            int64_t sum = 0;
            for (int i = 0; i < N; ++i) sum += static_cast<int64_t>(T[i * N + r]) * coeff[i * N + c];
            A[r * N + c] = clip16(roundShift(sum, shift1));
        }
    }

    std::vector<int32_t> X(N * N);
    for (int r = 0; r < N; ++r) {
        for (int c = 0; c < N; ++c) {
            int64_t sum = 0;
            for (int i = 0; i < N; ++i) sum += static_cast<int64_t>(A[r * N + i]) * T[i * N + c];
